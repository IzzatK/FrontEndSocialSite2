...
const getPosts = (req, res) => {
 const posts = Post.find().populate("postedBy", "_id name").select("__id title body created likes ").sort({created: -1})
 .then((posts) => {
     res.json(posts)
...
const postsByUser = (req, res) => {
    Post.find({postedBy: req.profile._id}).populate("postedBy", "_id name").select("__id title body created likes ").sort("_created").exec((err, posts) => {
        if(err) {
            return res.status(400).json({
                error: err
            })
...
module.exports = {
    getPosts, createPost, postsByUser, postsById, deletePost, isPoster, updatePost, photo, singlePost,
    like, unlike
};